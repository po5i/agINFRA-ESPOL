...
class FourStore_Store {
    ...
    public function add($graph, $turtle) {
        $s_sparql = "/sparql/";
        $s_data = "/data/";
        $d_endpoint=explode($s_sparql, $this->_endpoint);
        $post_endpoint = array_shift($d_endpoint) . $s_data; 
        $client = $this->client;
    ...
    public function update1($graph, $turtle) {
        $s_sparql = "/sparql/";
        $s_data = "/update/";
        $d_endpoint=explode($s_sparql, $this->_endpoint);
        $post_endpoint = array_shift($d_endpoint) . $s_data; 
    ...
    public function update($query) {
        ...
        $client = $this->client;
        $client->setUri( $post_endpoint);
        $client->setParameterPost('update', FourStore_Namespace::to_sparql() . $query);
        $response = $client->request('POST');
